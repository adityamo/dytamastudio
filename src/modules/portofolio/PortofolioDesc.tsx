  ...
  return (
    <div className="relative w-full space-y-5">
      <div className="space-y-2">
        ...
        <p className="text-gray-600 dark:text-neutral-200 text-xs lg:text-sm font-medium">
          {data.overview_description}
        </p>
        <ul className="list-disc list-inside text-gray-800 dark:text-white text-xs lg:text-sm">
          {data.the_solution.map((item: any, key: React.Key) => (
            <li key={key}>{item}</li>
          ))}
        </ul>
      </div>
      <div className="space-y-2">
        ...
        <ul className="list-disc list-inside text-gray-800 dark:text-white text-xs lg:text-sm">
          {data.the_challenge.map((item: any, key: React.Key) => (
            <li key={key}>{item}</li>
          ))}
        </ul>
      </div>
